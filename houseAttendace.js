...
function percentages(array){
    ...
    function percentages3(array){
        var elmt3= []
        array.forEach(member=>{
            if(member.party=="I" && member.votes_with_party_pct!== undefined){
...
return array.filter(member=>member.missed_votes_pct>=result);
}
least(allData)
missedVotes(allData)
...
statistics.independent=numbrep(allData, "I");
...
statistics.independent_votes_with_party_pct=percentage(allData, "I").toFixed(2);
statistics.votes_percentage=missedVotes(allData);
statistics.total_reps=numbrep(allData, "D") + numbrep(allData, "R") + numbrep(allData, "I")
statistics.total_votes=((percentage(allData, "D") + percentage(allData, "R") + percentage(allData, "I"))/3).toFixed(2)
...
function leastAttendace(allData){
    ...
    if(member.middle_name!== null){
        ...
        trow2.insertCell().innerHTML=fullname2.link(member.url);
         ...
         if(member.missed_votes!== null){
            console.log(member.last_name + " " + member.missed_votes)
            trow2.insertCell().innerHTML= member.missed_votes;
        }
        if(member.missed_votes==null){
    ...
    trow2.insertCell().innerHTML= member.missed_votes_pct;
    ...
    function mostAttendace(allData){
       ...
       if(member.middle_name!== null){
        ...
        trow5.insertCell().innerHTML=fullname2.link(member.url);
         ...
         if(member.missed_votes!== null){
            trow5.insertCell().innerHTML= member.missed_votes;
        }
        if(member.missed_votes==null){
            trow5.insertCell().innerHTML=0;
        } 
        trow5.insertCell().innerHTML= member.missed_votes_pct;    
